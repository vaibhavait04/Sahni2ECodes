...
{// Constructor for skip lists with keys smaller than largeKey and
 // size at most maxPairs. 0 < prob < 1.
   ...
   this->headerNode = new skipNode<K,E> (tailPair, maxLevel + 1);
   this->tailNode = new skipNode<K,E> (tailPair, 0);
   this->last = new skipNode<K,E> *[maxLevel+1];
...
{// Search for theKey saving last nodes seen at each
 ...
 // Return node that might contain theKey.
   ...
   for (int i = levels; i >= 0; i--)
   {
      while (beforeNode->next[i]->element.first < theKey)
         beforeNode = beforeNode->next[i];
      this->last[i] = beforeNode;  // last level i node seen
   }
...
{// Insert thePair into the dictionary. Overwrite existing
 // pair, if any, with same key.
   ...
   skipNode<K,E>* theNode = this->search(thePair.first);
...
{// Delete the pair, if any, whose key equals theKey.
   ...
   skipNode<K,E>* theNode = this->search(theKey);
   if (theNode->element.first != theKey) // not present
      return;

   // delete node from skip list
   for (int i = 0; i <= levels &&
                   this->last[i]->next[i] == theNode; i++)
      this->last[i]->next[i] = theNode->next[i];
   
   // update levels
   while (levels > 0 && this->headerNode->next[levels] ==this->tailNode)
      levels--;
...
{// Insert the dictionary pairs into the stream out.
   ...
   for (skipNode<K,E>* currentNode = this->headerNode->next[0];

...
class hashChains : public dictionary<K,E>
{
   public:
      ...
      pair<const K, E>* find(const K& theKey) const
         {return table[hashVar(theKey) % divisor].find(theKey);}

      void insert(const pair<const K, E>& thePair)
      {
         int homeBucket = (int) hashVar(thePair.first) % divisor;
      ...
      void erase(const K& theKey)
         {table[hashVar(theKey) % divisor].erase(theKey);}
   ...
   protected:
      ...
      hash<K> hashVar;              // maps type K to nonnegative integer

...
{
   // initialize
   std::cout << "Enter number of matrices" << endl;
   int q;
   std::cin >> q;
   r = new int [q + 2];
   ...
   for (int i = 1; i <= q; i++)
   {
      std::cout << "Enter number of rows in matrix " << i << endl;
      std::cin >> r[i];
   }
   std::cout << "Enter number of columns in matrix " << q << endl;
   std::cin >> r[q + 1];

   // compute cost of best way to multiply
   std::cout << "Minimum cost is " << c(1, q) << endl;

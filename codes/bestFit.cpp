...
int main(void)
{
   std::cout << "Enter number of objects and bin capacity" << endl;
   int numberOfObjects, binCapacity;
   std::cin >> numberOfObjects >> binCapacity;
   if (numberOfObjects < 2)
   {
      std::cout << "Too few objects" << endl;
      exit(1);
   }
   ...
   {
      std::cout << "Enter space requirement of object " << i << endl;
      std::cin >> objectSize[i];
      if (objectSize[i] > binCapacity)
      {
        std::cout << "Object too large to fit in a bin" << endl;
        exit(1);
      }
   }

...
{// Input the wire routing data.
   ...
   std::cout << "Enter the start position" << endl;
   std::cin >> start.row >> start.col;

   std::cout << "Enter the finish position" << endl;
   std::cin >> finish.row >> finish.col;

   // create and input the wiring grid array
   make2dArray(grid, size + 2, size + 2);
   std::cout << "Enter the wiring grid in row-major order" << endl;
...
void outputPath()
{
   std::cout << "The wire path is" << endl;
...
int main()
{
   ...
   if (findPath())
      ...
      std::cout << "There is no wire path" << endl;

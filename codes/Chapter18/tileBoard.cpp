...
{// Tile a defective chessboard.
   ...
   std::cout << "Enter k, board size is 2^k" << endl;
   std::cout << "k should be in the range 0 through 6" << endl;
   int k;
   std::cin >> k;
   if (k < 0 || k > 6)
   {
      std::cout << "k out of range" << endl;
      exit(1);
   }
   int size = 1;
   for (int i = 1; i <= k; i++)

...
int main()
{
   ...
   if (n < 2)
   {
      std::cout << "Too few elements" << endl;
      return 1;   // terminate with error
   }

   std::cout << "Enter number of relations" << endl;
   std::cin >> r;
   if (r < 1)
   {
      std::cout << "Too few relations" << endl;
      return 1;   // terminate with error
   }
   ...
   for (int i = 1; i <= r; i++)
   {
       std::cout << "Enter next relation/pair" << endl;
       std::cin >> a >> b;
       list[a].push(b);
       list[b].push(a);
   }
   ...
   std::cout << "End of list of equivalence classes" << endl;

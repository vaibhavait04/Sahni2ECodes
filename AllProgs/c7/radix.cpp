...
// Sorting within a key is done using a bin sort.
{
   ...
   int i, j, k, q; 
   for (i = 1; i <= n; i++) list[i].link = i+1; // link into a chain starting at @current@
   list[n].link = 0; int current = 1;
   for (i = d-1; i >= 0; i--) // sort on key @key[i]@
   {
      for (j = 0; j < radix; j++) f[j] = 0; // initialize bins to empty queues
	 ...
	 e[k] = current;
      ...
      for (k = j+1; k < radix; k++) // concatenate remaining queues
	  if (f[k]) {
	      list[last].link = f[k];
	      last = e[k];
	  }

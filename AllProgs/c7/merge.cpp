...
{
   int i1, i2, iResult; 
   for (i1 = l, i2 = m+1, iResult = l; // @i1@, @i2@, and @iResult@ are positions in the three lists
	i1 <= m && i2 <= n;
	iResult++)
   ...
   int t;
   if (i1 > m)
       for (t = i2; t <= n; t++) mergedList[iResult+t-i2] = initList[t];
   else
       for (t = i1; t <= m; t++) mergedList[iResult+t-i1] = initList[t];
}
...
merged from list @initList@ to list @resultList@. @n@ is the number of records in @initList@ */
{
   ...
   else for (t = i; t <= n; t++) resultList[t] = initList[t];
}
...
// Sort list @list@ into nondecreasing order of the keys @list[1].key@, ..., @list[n].key@.
{
	...
	for (m = 1; m <= 10; m++) cout << tempList[m].getKey() << " , "; cout << endl;
	l *= 2;
